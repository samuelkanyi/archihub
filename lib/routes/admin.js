...
router.put('/company', (req, res) => {
    CompanyModel.update(req.body, {
        ...
        }).then(update => res.status(301).redirect('/admin/images'))
...
//post requests
router.post('/project', upload.array('avatar'), (req, res) => {
        ...
        .then(project => {
            req.files.forEach(image => {
                new ImagesModel({
                    ...
                    projectId: project.getDataValue('id')
                }).save()
            })
        })
        .then(() => res.status(201).redirect('/admin/images'))
        .catch(err => console.error(err))
})
...
router.post('/images/finish', upload.single('avatar'), (req, res) => {
    ...
    new ImagesModel(req.body)
        .save()
        .then(fulfill => res.redirect('/admin/images'))
        .catch(err => console.error(err))
});
